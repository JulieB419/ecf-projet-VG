...
<form class="row g-2 mb-4" method="get">
  ...
  <div class="col-md-4 d-flex gap-2">
    ...
    <a class="btn btn-outline-secondary" href="<?= htmlspecialchars(url('espace-employe/horaires'), ENT_QUOTES, 'UTF-8') ?>">Horaires</a>
  </div>
</form>
...
<div class="list-group mb-5">
  <?php foreach ($orders as $o): ?>
    <a class="list-group-item list-group-item-action" href="<?= htmlspecialchars(url('espace-employe/commandes'), ENT_QUOTES, 'UTF-8') ?>/<?= (int)$o['id'] ?>">
    ...
    <div class="col-md-6 col-lg-4">
      <div class="card h-100"><div class="card-body">
        ...
        <div class="d-flex gap-2">
          <form method="post" action="<?= htmlspecialchars(url('espace-employe/avis'), ENT_QUOTES, 'UTF-8') ?>/<?= (int)$r['id'] ?>/valider">
            <input type="hidden" name="csrf" value="<?= htmlspecialchars(Session::csrf()) ?>">
            <button class="btn btn-sm btn-success">Valider</button>
          </form>
          <form method="post" action="<?= htmlspecialchars(url('espace-employe/avis'), ENT_QUOTES, 'UTF-8') ?>/<?= (int)$r['id'] ?>/refuser">
            <input type="hidden" name="csrf" value="<?= htmlspecialchars(Session::csrf()) ?>">
